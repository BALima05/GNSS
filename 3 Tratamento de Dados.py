...
FATORES = {
    2022: {
        'GPS': {'lat': 0.000000761, 'lon': -0.00000457},  # Aumentado em 100x
        'GLONASS': {'lat': 0.000000761, 'lon': -0.00000685},
        'GPS E GLONASS': {'lat': 0.000000761, 'lon': -0.00000533}
    },
    2023: {
        'GPS': {'lat': 0.00000304, 'lon': 0.0},
        'GLONASS': {'lat': 0.00000304, 'lon': 0.0},
        'GPS E GLONASS': {'lat': 0.00000304, 'lon': -0.000000761}
    }
}
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        for constelacao in CONSTELACOES:
            ...
            for idx, row in df.iterrows():
                ...
                # Obter fatores (zero para anos não especificados)
                fator = FATORES.get(ano, {}).get(constelacao, {'lat': 0, 'lon': 0})
                
                # Converter coordenadas
                lat_orig = dms_to_decimal(row['Latitude(gms)'])
                lon_orig = dms_to_decimal(row['Longitude(gms)'])
                
                # Armazenar valores decimais
                df_trat.at[idx, 'Latitude Decimal'] = lat_orig
                df_trat.at[idx, 'Longitude Decimal'] = lon_orig
                
                # Aplicar correções
                lat_ajust = lat_orig + fator['lat']
                lon_ajust = lon_orig + fator['lon']
                
                # Calcular diferenças em graus
                dif_lat = lat_ajust - lat_orig
                dif_lon = lon_ajust - lon_orig
                
                # Calcular diferenças em metros
                dx, dy = graus_para_metros(dif_lat, dif_lon, lat_orig)
                
                # Armazenar diferenças
                df_trat.at[idx, 'Latitude Ajustada'] = lat_ajust
                df_trat.at[idx, 'Dif. Latitude (graus)'] = dif_lat
                df_trat.at[idx, 'Longitude Ajustada'] = lon_ajust
